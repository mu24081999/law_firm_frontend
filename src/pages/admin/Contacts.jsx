...
function AdminContacts() {
  ...
  const [contacts, setContacts] = useState([
    {
      ...
      assignedTo: 'Law Firm A'
    }
  ]);
  const [selectedContact, setSelectedContact] = useState(null);
  const [filterTag, setFilterTag] = useState('');
  const [filterStatus, setFilterStatus] = useState('');
  const [filterLawFirm, setFilterLawFirm] = useState('');

  const lawFirms = ['Law Firm A', 'Law Firm B', 'Law Firm C']; // This would come from your backend
  ...
  const filteredContacts = contacts.filter(contact => {
    ...
    const matchesLawFirm = !filterLawFirm || contact.assignedTo === filterLawFirm;
    return matchesTag && matchesStatus && matchesLawFirm;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4">
        ...
        <select
          value={filterLawFirm}
          onChange={(e) => setFilterLawFirm(e.target.value)}
          className="rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
        >
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const updatedContacts = contacts.map(c =>
                          c.id === contact.id ? { ...c, assignedTo: e.target.value } : c
                        );
